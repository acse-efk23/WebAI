...
class MLOperator:
    ...
    def initialise(
        self, train_dataset, validation_dataset, batch_size=8, epochs=10
    ):
        ...
        self.model = ResNet(
            in_channels=self.train_dataset[0][0].shape[0],
            out_channels=self.train_dataset[0][1].shape[0],
        )
    ...
    def train_visual(self, epochs=5):
        ...
        return PlotLoss(self.history), Plot3D(
            y_pred.detach().numpy(),
            options={"filter": 2, "sample": 0.5},
    ...
    def train_one_epoch(self):
        ...
        return batch_loss / len(self.train_dataset), batch_r2 / len(
            self.train_dataset
        )
    ...
    def validate_one_epoch(self):
        ...
        with torch.no_grad():
            for x, y in self.validation_loader:
                ...
                batch_r2 += self.r2_score(prediction, y).item() * x.size(0)
        return batch_loss / len(self.validation_dataset), batch_r2 / len(
            self.validation_dataset
        )
    ...
    def load_json(self, path="dict.json"):
        if os.path.exists(path):
            with open(path, "r") as file:
                ...
                print(
                    f"Loaded features: {self.features}, time steps: {self.time_steps}, grid shape: {self.grid_shape}"
                )
