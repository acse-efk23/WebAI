...
class MinMaxNormalise:
    def __call__(self, np_array):
        num_features = np_array.shape[
            1
        ]  # Get the number of features from the second dimension
                        ...
                        for l in range(shape[4]):
                            if np.isnan(np_array[case, i, j, k, l]):
                                cube = np_array[
                                    case,
                                    i,
                                    max(0, j - half_size) : min(
                                        shape[2], j + half_size + 1
                                    ),
                                    max(0, k - half_size) : min(
                                        shape[3], k + half_size + 1
                                    ),
                                    max(0, l - half_size) : min(
                                        shape[4], l + half_size + 1
                                    ),
                                ]
                                if np.isnan(cube).all():
                                    output_array[case, i, j, k, l] = np.nanmean(
                                        np_array[case, i, :, :, :]
                                    )
                                else:
                                    output_array[case, i, j, k, l] = np.nanmean(
