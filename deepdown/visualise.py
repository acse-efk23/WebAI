...
def Plot3D(values, options=None, title=None):
    ...
    df = pd.DataFrame({"x": x, "y": y, "z": z, "values": values_flat})
    ...
    if "filter" in options:
        ...
        df = df[
            df["values"]
            > df["values"].mean() + num_stddevs * df["values"].std()
        ]
...
def PlotComparison(y, y_pred):
    ...
    fig.update_layout(
        height=500, width=1000, title_text="Comparison", showlegend=False
    )
